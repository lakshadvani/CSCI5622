...
class Knearest:
    ...
    def majority(self, item_indices):
        ...
        ind = self._y[item_indices]
        most = []
        most_common_ind = Counter(ind).most_common()
        values_ind=Counter(ind).values()
        #print(most_common_ind)
        for x in most_common_ind:
        	if x[1] == max(values_ind):
        		most.append(x[0])
    ...
    def classify(self, example):
        ...
        b = np.reshape(ind, (np.product(ind.shape),))

        return self.majority(b)
    ...
    def confusion_matrix(self, test_x, test_y):
        ...
        for test_ind, test_labels in zip(test_x, test_y):
            try:
                d[test_labels][self.classify(test_ind)] = 1+d[test_labels][self.classify(test_ind)]
            except:
                d[test_labels][self.classify(test_ind)] = 1
...
if __name__ == "__main__":
    ...
    print("train y shape",data.train_y.shape)
    
    print("test x shape",data.test_x.shape)
    print("test y shape",data.test_y.shape)
    '''
